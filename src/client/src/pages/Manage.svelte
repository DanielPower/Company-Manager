<script>
  ...
  const getEmployees = () => axios.get("/employees").then(({ data }) => {
    employees = data;
  });
  ...
  const getJobs = () => axios.get("/jobs").then(({ data }) => {
    jobs = data;
    console.log(jobs);
  });
...
<Content>
  <h1>Employees</h1>
  <DataTable headers={employeeTableHeaders} rows={employees} sortable="true" />
  <h1>Add employee</h1>
  ...
  <h1>Jobs</h1>
  <DataTable headers={jobTableHeaders} rows={jobs} sortable="true" />
  <h1>Add job</h1>
  <Form on:submit={submitJob}>
    ...
    <Dropdown bind:value={jobHourType} labelText="Job Hour Type" />
  </Form>
